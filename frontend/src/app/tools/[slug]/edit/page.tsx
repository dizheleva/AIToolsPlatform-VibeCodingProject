...
export default function EditToolPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (toolResponse.success && toolResponse.data) {
        ...
        if (storedUser) {
          ...
          const canEdit = 
            ...
            (toolData.creator?.id === userData.id && userData.status === 'approved');

          if (!canEdit) {
            toast({
              title: 'Нямаш права',
              description: 'Нямаш права да редактираш този инструмент.',
              variant: 'destructive',
            });
        ...
        setFormData({
          ...
          pricing_model: toolData.pricing_model || 'free',
          category_ids: toolData.categories?.map(c => c.id) || [],
      ...
      toast({
        title: 'Грешка',
        description: 'Възникна грешка при зареждане на данните.',
        variant: 'destructive',
      });
      router.push('/tools');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await toolsApi.update(slug, formData);
      if (response.success) {
        toast({
          title: 'Успешно!',
          description: 'Инструментът беше обновен успешно.',
          variant: 'success',
        });
        setTimeout(() => {
          router.push(`/tools/${slug}`);
        ...
        toast({
          title: 'Грешка',
          description: response.message || 'Грешка при обновяване на инструмент',
          variant: 'destructive',
        });
      }
    } catch (err: any) {
      toast({
        title: 'Грешка',
        description: err.message || 'Грешка при обновяване на инструмент',
        variant: 'destructive',
      });
    } finally {
      setSaving(false);
      ...
      <div className="mb-6">
        <Link href={`/tools/${slug}`}>
          <Button variant="ghost" className="mb-4">
            ...
            Назад към инструмент
          </Button>
        </Link>
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Редактирай AI инструмент</h1>
        <p className="text-gray-600">Обнови информацията за инструмента</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Обнови основната информация за инструмента</CardDescription>
            </CardHeader>
